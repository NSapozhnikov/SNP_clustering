...
for num_chr in range(1, 23):
    ### Reading all rows from  the table in DB to the list
    connection = connectToBD_MySQL()
    cursor = connection.cursor()
    engine = create_engine(establish_engine())
    try:
        ### This block needs to be run for updating LOC column in
        ### the corresponding table
        print(f"Using chromosome {num_chr}/22...")
        upd_query = f"""UPDATE clusters_chr{num_chr}_sq c
                        JOIN SNP_LOCATIONS u ON c.snp = u.snp
                        SET c.LOC = u.LOC;"""
        with engine.begin() as conn:
            print('Joinning SNP locations...')
            conn.execute(upd_query)
        ###    
        read_query = f"""SELECT SNP, hdbscan, dbscan, kmeans, spectral, LOC
                         FROM clusters_chr{num_chr}_sq
                         WHERE (hdbscan NOT LIKE -1 AND dbscan NOT LIKE -1)
                         ORDER BY CAST(hdbscan AS DECIMAL) ASC, 
                                  CAST(spectral AS DECIMAL) ASC, 
                                  CAST(dbscan AS DECIMAL) ASC, 
                                  CAST(kmeans AS DECIMAL) ASC, 
        ...
        for i in range(len(clusters__list)):
            key = clusters__list[i][1:5]
            ## Append (SNP, LOC) tuple by unique keys
            if not key in counter:
                counter[key] = []
            counter[key].append([clusters__list[i][0], clusters__list[i][5]])
        connection.commit()
        ...
        with open(f"chr{num_chr}.hlist", 'w') as fp:
        clusters = 0
        for key in counter:
