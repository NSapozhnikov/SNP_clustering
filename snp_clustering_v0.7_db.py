    ...
    with open(f"{out_file_path}_dbscan.clustered", 'w') as fp:
        ...
        for i in range(-1, n_clusters_):
            ...
            for j in cluster_indices:
                ...
                fp.write(str(i))
                fp.write('\n') 
                add_query = f"""UPDATE clusters_chr{num_chr} 
                                SET clusters_chr{num_chr}.dbscan = '{str(i)}'
    ...
    with open(f"{out_file_path}_hdbscan.clustered", 'w') as fp:
        ...
        for i in range(-1, n_clusters_):
            ...
            for j in cluster_indeces:
                ...
                fp.write('\n')
                add_query = f"""UPDATE clusters_chr{num_chr} 
                                SET clusters_chr{num_chr}.hdbscan = '{str(i)}'
                                WHERE SNP = '{str(snp_list[j])}';"""
                cursor.execute(add_query)
                connection.commit()
    ...
    with open(f"{out_file_path}_kmeans.clustered", 'w') as fp:
        ...
        for i in range(n_clusters_):
            ...
            for j in cluster_indeces:
                ...
                fp.write('\n')
                add_query = f"""UPDATE clusters_chr{num_chr} 
                                SET clusters_chr{num_chr}.kmeans = '{str(i)}'
                                WHERE SNP = '{str(snp_list[j])}';"""
                cursor.execute(add_query)
                connection.commit()
    ...
    with open(f"{out_file_path}_spectral.clustered", 'w') as fp:
        ...
        for i in range(n_clusters_):
            ...
            for j in cluster_indeces:
                ...
                fp.write('\n') 
                add_query = f"""UPDATE clusters_chr{num_chr} 
                                SET clusters_chr{num_chr}.spectral = '{str(i)}'
                                WHERE SNP = '{str(snp_list[j])}';"""
                cursor.execute(add_query)
                connection.commit()
                ...
                try:
                    with open (snp_file_path, 'r') as snp_file: 
                        ...
                        cursor.execute(f"""SELECT * 
                                           ...
                                           WHERE table_schema = 'SNP_Clusters' 
                        ...
                        if cursor.fetchone()[0] == 0:
                            for snp in snp_list:
                                cursor.execute(f"""INSERT INTO snp_clusters.clusters_chr{num_chr} (SNP) VALUES ('{snp}')""")
                                connection.commit()
